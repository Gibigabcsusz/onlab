...
int main(void)
{
    // Bemenetek megadása
    const int T = 2200;
    const int Ta = 2200; // az ábrázolás időlépésének száma, min=2
    const int Ng = 300;
    const int Nc = 15;
    const int Np = Nc*Ng;
    const double maxvin = 0.1;
    const double omDT = 0.2;
    const double fihSzorzo = 1;
    const long int seedNum = 1;
    const double xAblakA = 0.0;
    const double xAblakF = 0.0;
    const int rhoAblak = 6;
    ...
    uniform_real_distribution<double> unifx(-0.5+Ng*xAblakA, Ng-0.5-Ng*xAblakF);
    ...
    for(t=1;t<T+1;t++)
    {
        ...
        if(t==Ta)
        {
            ...
            for(i=0; i<Ng+1; i++)
                for(int j=i-rhoAblak; j<i+1+rhoAblak; j++)
                    rhoa[i]+=rho[(j%Ng+Ng)%Ng]/(2*rhoAblak+1);
            filePrinter(Ng+1, sorozat, rhoa, "output/rho.dat", "X", "Töltéssűrűség", "rho");
        }
        //TODO
        for(i=0; i<Ng; i++)
        {
